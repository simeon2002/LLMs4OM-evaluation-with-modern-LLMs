...
**File:** `ontomap/ontology/bioml.py`, `ontomap/base/ontology.py`  
**Trigger in pipeline:** `task_obj.load_from_json()` or `task_obj.collect()`

OWL ontology files are parsed with `owlready2`. For each class with a label:
- Extracts: `iri`, `label`, `parents`, `children`, `synonyms`, `comment`

Result — `task_owl` dict:
```python
{
  "source": [{"iri": "...", "label": "disease", "parents": [...], "childrens": [...]}],
  "target": [{"iri": "...", "label": "disease", ...}],
  "reference": {"equiv": {"full": [{"source": iri, "target": iri}]}}
}
```
