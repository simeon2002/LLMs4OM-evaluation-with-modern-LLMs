...
if model_name is None:
    ...
    print("Usage: python pipeline_retrieval.py --model <model_name>")
    sys.exit(1)

from ontomap.ontology_matchers import MatcherCatalog
from ontomap.pipeline.om_pipeline import OMPipelines

if model_name not in MatcherCatalog["retrieval"]:
    print(f"Unknown model '{model_name}'. Available: {list(MatcherCatalog['retrieval'].keys())}")
    sys.exit(1)
...
config = {
    ...
    "approach-encoders-to-consider": ["label-children"],
    "do-evaluation": False,
    "load-from-json": True,
    "root_dir": "datasets",
    "device": "cuda",
